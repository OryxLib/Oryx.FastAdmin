...
from akshare.stock.cons import (zh_sina_a_stock_payload,
                                zh_sian_hq_nodes,
                                zh_sina_a_stock_url,
                                zh_sina_a_stock_bypage_url,
                                zh_sina_a_stock_bynode_count_url,
...
def get_hy_nodes():
    ...
    if(allHQData): 
        return allHQData
    res = requests.get(zh_sian_hq_nodes)
    jsonData = json.loads(res.text.replace('\'','\"'))
    allHQData = jsonData[1][0][1]
    return allHQData
    
"""
    所有行业 依据新浪申万行业分类
"""
def get_zh_a_stock_hy():
    hangye = get_hy_nodes()[2] 
    hangye_level_1_list = hangye[1]
    hyList=[]
    """
        根据节点数据解析行业数据
    """
    for item in hangye_level_1_list:
        level_1_name = item[0]
        level_1_code = item[3]
        for level_1_item in item[1]:
            level_2_name= level_1_item[0]
            level_2_code= level_1_item[2]
            hyList.append((level_1_name,level_1_code,level_2_name,level_2_code))
    return hyList

"""
    所有热门概念 
"""
def get_zh_a_hot_gainian():  
    hotGainian = get_hy_nodes()[3][1]
    hotGainianList= []
    for level_1_item in hotGainian:
        item_name = level_1_item[0]
        item_code = level_1_item[2]
        hotGainianList.append((item_name,item_code))
    return hotGainianList

"""
    所有普通概念 
"""
def get_zh_a_gainian():
    gainian=get_hy_nodes()[4][1]
    gainianList=[]
    for level_1_item in gainian:
        item_name = level_1_item[0]
        item_code = level_1_item[2]
        gainianList.append((item_name,item_code))
    return gainianList

"""
    所有地域
"""
def get_zh_a_diyu(hq_a):
    diyu=hq_a[5][1]
    diyuList=[]
    for level_1_item in diyu:
        item_name = level_1_item[0]
        item_code = level_1_item[2]
        diyuList.append((item_name,item_code))
    return diyuList


def get_zh_a_stock_byQueryList(queryList):
    """
    query item format : ('病毒检测','chgn_700867')
    """
    stockList=[]
    for query in queryList:
        stockList += get_zh_a_stocklist_byQueryOne(query[1])
    return stockList
    
def get_zh_a_stocklist_byQueryOne(id):
    """
    Data format : 
    ...
    per: 39.449
    pb: 2.506
    mktcap: 396542.789497
    nmc: 369629.394671
    turnoverratio: 8.11157

    list format :
    0: {symbol: "sh603716", code: "603716", name: "塞力斯", trade: "19.330", pricechange: "0.230",…}
    1: {symbol: "sh688068", code: "688068", name: "热景生物", trade: "85.500", pricechange: "14.250",…}
    2: {symbol: "sh688399", code: "688399", name: "硕世生物", trade: "117.480", pricechange: "19.580",…}
    3: {symbol: "sz002022", code: "002022", name: "科华生物", trade: "17.790", pricechange: "1.620",…}
    4: {symbol: "sz002030", code: "002030", name: "达安基因", trade: "16.400", pricechange: "1.490",…}
    5: {symbol: "sz002584", code: "002584", name: "西陇科学", trade: "7.060", pricechange: "0.640",…}
    6: {symbol: "sz300206", code: "300206", name: "理邦仪器", trade: "8.140", pricechange: "0.660",…}
    7: {symbol: "sz300482", code: "300482", name: "万孚生物", trade: "62.600", pricechange: "1.800",…}
    8: {symbol: "sz300676", code: "300676", name: "华大基因", trade: "87.450", pricechange: "7.950",…}
    9: {symbol: "sz300683", code: "300683", name: "海特生物", trade: "27.500", pricechange: "-3.050",…}
    """
    #res = requests.get(zh_sina_a_stock_count_url )  
    res = requests.get(zh_sina_a_stock_url.format(id)) 
    jsonData = demjson.decode(res.text) 
    return jsonData

#def get_zh_a_stockData_byId(Id):
...
def stock_zh_a_spot_by_page(page=1):
    big_df = pd.DataFrame()
    ...
    big_df = pd.DataFrame(data_json)
    return big_df

def stock_zh_a_bill(symbol):
    print('方法未实现')
    #http://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/CN_Bill.GetBillListCount?symbol=sh600900&num=60&page=1&sort=ticktime&asc=0&volume=0&amount=2000000&type=0&day=today-11 
    pass

def stock_zh_a_billCount(symbol):
    print('方法未实现')
    #http://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/CN_Bill.GetBillListCount?num=100&page=3624&sort=ticktime&asc=0&volume=40000&type=0
    pass
...
def stock_zh_a_spot():
    ...
    for page in range(1, page_count+1):
        print(page)
        big_df= stock_zh_a_spot_by_page(page)
