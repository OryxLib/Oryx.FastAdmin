    ...
    class ProjectSaverComponent extends React.Component {
        ...
        componentDidUpdate(prevProps) {
            ...
            if (this.props.isRemixing && !prevProps.isRemixing) {
                console.log('project saver hoc - onRemixing')
        ...
        updateProjectToStorage() {
            ...
            console.log(this.props.reduxProjectId)
        ...
        download(data, filename, type) {
            try {
                ...
                console.log('asset exception', exc)
            }
        }
        wrapAssetData(asset) {
            if (!window['step']) {
            ...
            window['step']++;
            let token = '';
            let key = `asset/${asset.assetId}.${asset.dataFormat}`;
            let xhrResult = xhr({
                url: `/QiniuFile/TokenByKey?fileName=${key}`,
                method: 'get',
                sync: true,
            ...
            //console.log(`asset ${window['step']}`, asset)
            let blob = new Blob([asset.data]);
            //this.download(blob, `${window['step']}-${asset.assetId}.${asset.dataFormat}`, 'image/png')
            //console.log(`end download asset ${window['step']}`, asset)
            let formData = new FormData();
            formData.append('token', token)
            formData.append('file', blob)
            formData.append('key', `asset/${asset.assetId}.${asset.dataFormat}`)
            return formData;
        }
        /**
         * storeProject:
        ...
        storeProject(projectId, requestParams) {
            console.log('storeProject projectId')
        ...
        storeProjectThumbnail(projectId) {
            try {
                this.getProjectThumbnail(dataURI => {
                    console.log('store project thumbnail')
                    this.props.onUpdateProjectThumbnail(projectId, dataURI, dataURItoBlob(dataURI));
                });
            } catch (e) {
