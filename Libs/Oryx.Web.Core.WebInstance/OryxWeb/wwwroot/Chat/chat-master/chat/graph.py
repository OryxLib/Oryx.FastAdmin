...
class Database():
    ...
    def __init__(self, password="train", userid="A0001"):
        ...
        self.user = self.selector.match("User", userid=userid).first()
        if not self.user:
    ...
    def add_nlucell(self, label="NluCell", name=None, content=None, topic="", tid="", \
        ...
        for question in name.split(delimiter):
            ...
            if question: # 问题不能为空
                ...
                node = self.selector.match("NluCell").where("_.name ='" + question + "'", \
    ...
    def handle_excel(self, filename=None, custom_sheets=[]):
        ...
        for sheet_name in sheet_names:
            ...
            # DeprecationWarning: Graph.find_one is deprecated, use NodeMatcher instead. 2017-5-18
            # config_node = self.graph.find_one("Config", "name", sheet_name)
            config_node = self.selector.match("Config", name=sheet_name).first()
            if not config_node:
                # 默认 self.user 已存在
    ...
    def download(self, filename=None, names=[]):
        ...
        for name in names:
            subgraph = self.selector.match('Config', name=name).first()
