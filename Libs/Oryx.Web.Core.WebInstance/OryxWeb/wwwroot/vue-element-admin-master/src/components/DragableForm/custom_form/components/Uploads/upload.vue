<template>
  <div> 
        ...
        <template v-if="item.status == 'finished'">
          <p style="display: inline-block">
            <a
              ...
              :href="item.url"
              target="_blank"
            >{{item.name}}{{!!item.file_name && (item.name == item.file_name?"":'('+item.file_name+')')}}</a>
            <el-input
              @enter="handleChangeAlias"
              autofocus
              placeholder="请设置附件别名"
              v-else
    ...
    <!-- <el-dialog title="图片属性设定" v-model="visible" @ok="handleCloseModal">
      <img :src="image_url" v-if="visible" style="width: 100%">
      <el-form :model="formData" label-width="80" style="margin-top: 24px;">
        <el-form-item label="附件别名：">
...
export default {
  ...
  methods: {
    handleChangeAlias() {
      this.uploadList[this.formData.index][
        "file_name"
    ...
    handleFormatError(file) {
      this.$Notice.warning({
        title: "文件格式不正确",
        desc: "文件：" + file.name + "格式不正确, 请选择 jpg/png."
      });
    },
    handleMaxSize(file) {
      this.$Notice.warning({
