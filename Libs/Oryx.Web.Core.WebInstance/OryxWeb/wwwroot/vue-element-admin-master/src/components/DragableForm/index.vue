<template>
  <div class="container">
    <el-row>
      ...
      <el-dialog v-model="showModal" :title="'配置' + modalFormData.modalTitle + '属性'" :mask-closable="false">
        <el-form class="form_content" label-width="80" :model="modalFormData" ref="modalFormData">
          ...
          <el-form-item label="数据字典：" v-if="showModal">
            <el-select v-model="modalFormData.dict" filterable @change="handleDataDictChange">
              ...
              <el-option :disabled="dataDictSelected.indexOf(item.id) >= 0" v-for="item in dataDictList" :value="JSON.stringify({
                id: item.id, parent_name: item.parent_name})" :key="item.id">{{ item.label }}</el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="关联配置：" v-if="typeof modalFormData.relation != 'undefined' && modalFormData.relation">
            <el-select v-model="modalFormData.relation_name" class="inline-block" style="width: 150px" @change="_=>modalFormData.relation_value = ''">
