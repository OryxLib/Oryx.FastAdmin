<template>
  <div>
    <el-row>
      <el-col :span="12" class="sortable_container">
        <el-form label-width="100" class="b-a">
      ...
      <el-col :span="12" class="sortable_item">
        ...
        >
          <el-alert style="margin: 15px 15px 0;" type="warning" show-icon>未绑定数据字典控件无效</el-alert>
          ...
          <el-form-item>
            ...
            <el-button type="ghost" @click="handleReset()" style="margin-left: 8px">Reset</el-button>
          </el-form-item>
        </el-form>
      </el-col>
      <el-dialog
        ...
        :mask-closable="false"
      >
        <el-form class="form_content" label-width="80" :model="modalFormData" ref="modalFormData">
          ...
          <el-form-item label="name属性：" v-if="typeof modalFormData.name != 'undefined'">
            <el-input v-model="modalFormData.name" placeholder disabled></el-input>
          </el-form-item>
          ...
          >
            <el-select
              ...
              class="inline-block"
              style="width: 150px"
              @change="_=>modalFormData.relation_value = ''"
            >
              <el-option
          ...
          <el-form-item label="最大长度：" v-if="typeof modalFormData.maxLength != 'undefined'">
            <el-input-number v-model="modalFormData.maxLength" placeholder="请输入文本限制最大长度"></el-input-number>
          </el-form-item>
...
import form_list from "../../components/DragableForm/custom_form/FormList";
import {
  loadLableJson,
  loadTableSchem,
  loadTableFormMapping,
  insertTableForm
...
export default {
  ...
  created() {
    ...
    loadTableSchem(this.tableName)
      ...
      .then(() => {
        loadTableFormMapping(this.tableName)
          .then(res => {
            this.tableFormList = res.data;
          })
          .then(() => {
            this.tableItemList.forEach(item => {
              var formItem = this.tableFormList.find(x => x.Form == item.Name);
              if (formItem) { 
                this.sortable_item.push( formItem.FormConfig);
              } else { 
                var eleConf = form_list.find(x => x.ele == "input");
                eleConf= JSON.parse(JSON.stringify(eleConf))
                eleConf.obj.label=item.Name;
                eleConf.obj.name = item.Name;
                eleConf.bindColumn = item.Name
                eleConf.bindTable = item.Table
                this.sortable_item.push(eleConf);
              }
            });
          });
      });
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      insertTableForm(this.sortable_item)
